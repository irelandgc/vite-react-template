...
| `PlanDefinition-<examSite>.json` | Published structure verbatim: timeframe rows as actions with priority codes, nested `selectionBehavior` for compound items, badges, `documentation` with `source-page` (or draft reference), guidance, sources, HealthPathways page-ID placeholder. **Every logic-carrying action inside an indication block (code `criteria-block: indication`) carries an `indication-theme` extension** — `{ url: "http://crr.health.nz/fhir/StructureDefinition/indication-theme", valueCodeableConcept: { coding: [{ system: "http://crr.health.nz/fhir/CodeSystem/indication-theme", code, display }] } }` — so the Viewer's indication-first grouping (CV-027) is in the artefact, not in page code. Use the theme the presentation *is* (how a GP frames it), not the urgency; "Specialist-endorsed referral" for the specialist-advice pathway; no "Other". `check` rule 7b enforces presence. See `instructions/archive/indication-groupings.md` for the vocabulary of themes established across the pre-migration Viewer. |
...
5. **Build gates.** From `tooling/criteria-bundle/tooling`: `npm run build && npm test && npm run check` green. The check must show the new site's Questionnaire linkIds all resolve, every logic-carrying action has a page or draft reference, and every logic-carrying action inside an indication block has an `indication-theme`.
