...
1. **Read the source and list the atoms.** Before writing CQL, produce in `transcription-notes.md` a table of every clinical fact the criteria mention (symptom, finding, investigation, threshold, duration, age/sex band, gateway, exclusion) → the indicator that will represent it (existing vocabulary linkId or new). This table is what the reviewer checks first.
