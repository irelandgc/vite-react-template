/**
 * Triage Advisor v2.2.0 Regression Test Runner
 * Fixes from v2.1.0 test:
 *   1. Paediatric detection — uses paed_index for under-16 patients (fixes RP-005, CR-002)
 *   2. Tests against v2.2.0 prompt (concrete gateway examples + postmenopausal shorthand)
 */
...
async function main() {
  console.log('Fetching v2.2.0 system prompt from D1...');
  ...
  if (ADMIN_KEY) {
    ...
    promptText = readFileSync('/Users/garyireland/vite-react-template/instructions/system-prompt-v2.2.0.txt', 'utf8');
    console.log('  No ADMIN_KEY — reading v2.2.0 from local file');
  }
  ...
  const jsonPath = '/Users/garyireland/vite-react-template/instructions/prompt-v2.2.0-test-results.json';
  ...
  if (regressions.length > 0) {
    ...
    for (const r of regressions) {
      ...
      lines.push(`**v2.2.0 verdict:** ${ar.verdict_title || ar.error || 'N/A'}`);
  ...
  const mdPath = '/Users/garyireland/vite-react-template/instructions/prompt-v2.2.0-test-results.md';
