...
**Status: IN PROGRESS — branch `feature/arch-mig-slice5-pipeline`. Deliverables 1–4 of 8 done and green (D1 `merge.ts`; D2 `POST /api/assess` + main-worker forward + migration 0010; D3 attestation-questions mechanism + `GET /api/assess/attestation-questions` + AD-23 + vocab 1.1.0; D4 `shared/advisory-render.js` two views + `bundleArtefacts` in the response). api-worker 140/140. Remaining: D5 thin Triage page behind the flag, D6 compare-extraction mode, D7 two-worker `wrangler dev` e2e on the four CT CAP notes, D8 registers/docs (SD-01 status, release-log draft, integration guide "pipeline mode"). Not yet done: nothing user-facing; flags off.**
