...
**Status: COMPLETE (engineering) — branch `feature/arch-mig-slice3-engine`; AD-19 (national library load model — **Proposed**, ship interim import, registry follow-up = **SR-13**), AD-20 (evaluate route contract — explicit `requestedExamSite`/`candidateExamSites`), AD-13 amended (immutable test alias); SD-11 (service binding + internal-only gating via `x-assess-internal` secret), SD-12 (audit record), SR-13 raised. `POST /api/assess/evaluate` reproduces all 17 non-record CT CAP scenarios and the red-flag precedence over HTTP under the AD-13 harness (51/51 api-worker tests), is internal-only (403 without the shared secret, 404 with the pipeline flag off), and the two-worker `wrangler dev -c wrangler.json -c public/crr-criteria/wrangler.json` round trip returns a correct `P2_URGENT` with an audit row. All flags off in committed config. Not done: `ASSESS_PIPELINE_ENABLED` must stay off until SR-13 (fail-closed national bundle) lands; the `assessment_notes` store awaits privacy-office sign-off (KI-34); migration 0009 remote apply is a slice 10 cut-over item.**
...
**Status: 4a COMPLETE — PR #7 (content: contract v2, national Questionnaire, prompt v3.0.0, equivalence list, ground-truth seed, gate vectors — AD-14/15/16). 4b not started (service: PII gate, `/api/assess/extract`, provider abstraction, gate implementation).**
...
| SR-13 | slice 3 (raised) | National red-flag / ACC safety layer shipped as a deploy-time import, not a governed published artefact (AD-19); registry-publish under `national-redflags` with the engine failing closed. `ASSESS_PIPELINE_ENABLED` stays off until closed. |
