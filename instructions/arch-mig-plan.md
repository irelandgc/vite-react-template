...
**Status: COMPLETE (PR open) — branch `feature/arch-mig-slice5-pipeline`. All 8 deliverables done and green.** D1 `merge.ts`; D2 `POST /api/assess` + main-worker forward + migration 0010; D3 attestation-questions mechanism + `GET /api/assess/attestation-questions` + AD-23 + vocab 1.1.0; D4 `shared/advisory-render.js` two views + `bundleArtefacts` in the response; **D5** thin Triage `<script type="module">` behind `GET /api/assess/status` (200 ⇒ thin client: note + context + attestations in, Advisory out; no prompt assembly / `EMBEDDED_MATCH_DATA` / synonym detect / post-processing / model call in the browser; flag-off path byte-identical — legacy code intact for slice 10); **D6** `POST /api/assess/compare-extract` (two providers/models, same contract, per-indicator diff + engine determination each; Azure run reported as an error, visibly); **D7** `run-pipeline-e2e.mjs` two-worker `wrangler dev` (`benchmark/results/2026-09-07-pipeline-e2e-claude-sonnet-4-6.md`): 3/4 CT CAP ground-truth determinations MATCH, MW-009 diverges on `workup.localisingFeatures` extraction (FINDINGS #6, open — extraction drift, not a wiring fault), one complete `assessments` row per call, AD-17 attestation flow proven end-to-end; **D8** this line + AD-23/SD-01 updated + release-log draft + integration-guide "pipeline mode" section. api-worker **148/148**; `wrangler deploy --dry-run` both workers; **all flags off in production config.**
