...
// ══════════════════════════════════════════════════════════════
//
// `prompt-v3.0.3.json` is canonical (the .md is a rendering of it; `npm run
...
// `{ linkId, value, status, quote }` plus `examSites[]` (v3.0.3). The route builds
// the FHIR QuestionnaireResponse and the answer-evidence extensions from that
// list — the model no longer hand-writes FHIR (SR-09, prompt-v3.0.3.md).
//
...
// The output tool (v3.0.3). Passed to the provider as the sole tool with a forced
...
// (`FALLBACK_INSTRUCTION_TEXT` is retired; prompt-v3.0.3.md "What is deliberately
// not in it").
if (!PROMPT_VERSION || !EQUIVALENCE_LIST_VERSION || !Array.isArray((promptV3 as any).parts) || !(promptV3 as any).parts.length) {
  throw new Error("extraction prompt failed to load — prompt-v3.0.3.json is missing version, equivalenceListVersion or parts[]");
}
if (!OUTPUT_TOOL || !OUTPUT_TOOL.name || !OUTPUT_TOOL.input_schema) {
  throw new Error("extraction prompt failed to load — prompt-v3.0.3.json is missing outputTool.name / outputTool.input_schema");
}
...
function contextBlock(ctx: AssessmentContext): string {
  ...
  // model (prompt v3.0.3, EVIDENCE rule 8). A supplied lab does NOT suppress
