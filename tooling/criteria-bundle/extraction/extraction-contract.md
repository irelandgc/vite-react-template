...
4. **Clinical interpretation is an inference.** "Tired all the time" → `symptom.fatigue = true` is documented (same concept). "Clothes loose" → `weightloss.present = true` is documented. "Clothes loose" → `weightloss.percent` is **not answerable** (omit). "Hb 98" → `lab.hb.low = true` is **inferred** unless the note says it is low or flags it. A single recorded weight ("wt 58") is **not weight loss**: it supports **neither `weightloss.present = true` nor `weightloss.present = false`** — one measurement says nothing about change in either direction. Omit the item; it needs a stated loss, or two weights to compare (this is rule 3's "a calculation needs both numbers" seen from the other side). Answering `false` here is rule 1's forbidden move (a `false` that means "not established"), and answering `false` as `documented` is worse. Benchmark findings — MW-009, "wt 58" answered `weightloss.present` in 2/3 runs as `inferred` (`benchmark/FINDINGS.md` item 7) and, once E-08 was added, in 1/3 runs as `false / documented` (item 10).
