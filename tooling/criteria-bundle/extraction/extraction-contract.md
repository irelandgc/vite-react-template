...
4. **Clinical interpretation is an inference.** "Tired all the time" → `symptom.fatigue = true` is documented (same concept). "Clothes loose" → `weightloss.present = true` is documented. "Clothes loose" → `weightloss.percent` is **not answerable** (omit). "Hb 98" → `lab.hb.low = true` is **inferred** unless the note says it is low or flags it. A single recorded weight ("wt 58") is **not weight loss**: `weightloss.present` is not answerable from one measurement — it needs a stated loss, or two weights to compare (omit; this is rule 3's "a calculation needs both numbers" seen from the other side). Benchmark finding — MW-009, "wt 58" answered `weightloss.present` in 2/3 runs (`benchmark/FINDINGS.md` item 7).
