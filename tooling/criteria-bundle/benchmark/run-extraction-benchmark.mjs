...
// (value / status / quote / answered), expectedAbsent false positives with the
// value/status/quote of each and whether `strict` would have excluded it, and
// the engine determination per run. The prompt and equivalence-list versions
// are read from the route response and stamped on the report. Model output is
// non-deterministic — one run is a sample, not a rate. A miss is a FINDING, not
// a failure of the slice. See benchmark/FINDINGS.md for the running log.
//
...
for (const file of files) {
  ...
  for (let r = 0; r < RUNS; r++) {
    ...
    if (ex.json.promptVersion) promptVersion = ex.json.promptVersion;
    if (ex.json.equivalenceListVersion) equivVersion = ex.json.equivalenceListVersion;
  ...
  // expectedAbsent false positives across runs — a false positive is a
  // fabrication (the ground truth says this item is not answerable from the
  // note). Record the value/status/quote so the report can say whether the
  // default `strict` engine would have excluded it (an `inferred` answer does
  // not establish a criterion under strict; a `documented` one does).
  const absent = [];
  for (const linkId of Object.keys(gt.expectedAbsent || {})) {
    const answeredIn = [];
    runs.forEach((run, i) => {
      const g = run.answers?.get(linkId);
      if (g) answeredIn.push({ run: i + 1, value: g.value, status: g.status, quote: g.quote });
    });
    absent.push({ linkId, hits: answeredIn.length, answeredIn, why: gt.expectedAbsent[linkId] });
  }
...
md += `Prompt **v${promptVersion}**, equivalence list **${equivVersion}**. `;
md += `Each note goes through \`/api/assess/extract\` (flat tool output — the service builds the FHIR response) then, on a gate pass, through \`/api/assess/evaluate\` (\`strict\` documentation standard). Model output is non-deterministic: the per-run split is a sample, not a stable rate. A miss is a finding.\n\n`;
...
md += `## Summary\n\n`;
md += `| | value |\n|---|---|\n`;
md += `| prompt / equivalence list | v${promptVersion} / ${equivVersion} |\n`;
...
for (const c of cases) {
  ...
  if (c.absent.length) {
    ...
    for (const a of c.absent) {
      md += `- \`${a.linkId}\`: ${tick(a.hits === 0 ? RUNS : RUNS - a.hits, RUNS)} clean`;
      if (a.hits) {
        md += ` — **answered in ${a.hits}/${RUNS}**`;
        for (const h of a.answeredIn) {
          const strict = h.status === "inferred" ? "excluded under strict" : h.status === "documented" ? "**counts under strict**" : `status ${h.status}`;
          md += `\n  - run ${h.run}: \`${h.value}\` / ${h.status} · quote ${JSON.stringify(h.quote)} · ${strict}`;
        }
      }
      md += `\n`;
    }
