...
- **DG-001** — note: `21y man, headche, known hx of met mel.` → in the original evaluation run (DB 178, v1.1.0/Sonnet 4) the assessment asserted **"cognitive impairment"**, absent from the note (Danielle: "the referral doesn't state anything about cognitive impairment"). In the current REG02-B run (v2.3.0/Sonnet 4.6) the phrase appears in the notes field naming an alternative qualifying pathway, not in met_criteria — the verdict failure (proceeds 3/3, expected declined) and fabrication flag persist.
...
**6.3 The override's contribution is unmeasured.** Step 3b (prompt) and `postProcessingValidation()` (client) are deliberate belt-and-braces for the same failure class. REG02 config C — the measurement of what the braces actually do on the current model — never ran. If v3 restructures Step 3b, that measurement (or a decision it isn't needed) should come first; run config C fresh — no partial config-C data exists on any branch (confirmed 7 Jul, `scripts/reg02-checkpoint.json` contains only completed configs A/B).
